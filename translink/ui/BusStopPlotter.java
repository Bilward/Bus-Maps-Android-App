    ...
    public void markStops(Location currentLocation) {
        ...
        for (Stop nextStop : StopManager.getInstance()) {
            ...
            if (Geometry.rectangleContainsPoint(northWest, southEast, nextStop.getLocn())) {
                GeoPoint stopGeoPoint = new GeoPoint(nextStop.getLocn().getLatitude(), nextStop.getLocn().getLongitude());
                String infoTitle = nextStop.getNumber() + " " + nextStop.getName();

                for (Route nextRoute : nextStop.getRoutes()) {
                    infoTitle = infoTitle + "\n" + nextRoute.getNumber();
                }

                if (getMarker(nextStop) == null) {
                    Marker newMarker = new Marker(mapView);
                    setMarker(nextStop, newMarker);

                    getMarker(nextStop).setRelatedObject(nextStop);
                    getMarker(nextStop).setIcon(stopIconDrawable);
                    getMarker(nextStop).setInfoWindow(stopInfoWindow);
                    getMarker(nextStop).setTitle(infoTitle);
                    getMarker(nextStop).setPosition(stopGeoPoint);

                    stopClusterer.add(newMarker);
                } else {
                    stopClusterer.add(getMarker(nextStop));
                }
            }
        }
    }
