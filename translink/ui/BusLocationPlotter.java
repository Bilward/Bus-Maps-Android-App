...
public class BusLocationPlotter extends MapViewOverlay {
    ...
    public void plotBuses() {
        ...
        if (StopManager.getInstance().getSelected() != null) {
            ...
            for (Bus next : buses) {
                GeoPoint busPoint = new GeoPoint(next.getLatLon().getLatitude(), next.getLatLon().getLongitude());
                OverlayItem busIcon = new OverlayItem("RouteNo:" + next.getRoute(),
                        "Dest:" + next.getDestination(), busPoint);
